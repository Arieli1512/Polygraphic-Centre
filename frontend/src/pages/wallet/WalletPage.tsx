...
export const WalletPage: React.FC = () => {
    ...
    const {data} = useQuery(createUserWalletQuery(client_id ?? -1));
    const queryClient = useQueryClient();
    const mutationWalletTopUp = useMutation({...createWalletTopUpMutation(),
        onSuccess: () => {
            queryClient.invalidateQueries({queryKey: [`GET_USER_WALLET_${client_id}`]})
        }});
    ...
    return (
        <main className={styles.walletMain}>
            <div className={styles.walletInfoContainer}>
                ...
                <div className={styles.walletBalance}> {data?.balance ?? 0}zł </div>
                <button onClick={() => {
